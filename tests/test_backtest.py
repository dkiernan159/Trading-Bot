...
def flat_bar(dt: datetime, price: float, spread: float = 0.5) -> Bar:
    ...
    can't form a gap, so it doesn't also register as a false 1m FVG."""
...
def breakout_15m_anchor_nested_1m_bars() -> list[Bar]:
    ...
    anchor (gap 105.3-109.1), then a small 1m FVG nested inside it (gap
    106.2-107.0) whose midpoint (106.6) is where the resulting limit
    order fills."""
    bars = [
    ...
    nested_start = pattern_start + timedelta(minutes=46)
    for i in range(5):
        bars.append(bar_at(nested_start + timedelta(minutes=i), 106.0, 106.2, 105.9, 106.0))  # baseline
    bars.append(bar_at(nested_start + timedelta(minutes=5), 106.0, 106.2, 105.9, 106.0))  # c0
    bars.append(bar_at(nested_start + timedelta(minutes=6), 106.0, 107.5, 105.9, 107.4))  # c1: displacement
    bars.append(bar_at(nested_start + timedelta(minutes=7), 107.4, 107.6, 107.0, 107.5))  # c2: confirms gap
    bars.append(bar_at(nested_start + timedelta(minutes=8), 107.5, 107.6, 107.4, 107.5))  # flush -- detects it
    bars.append(bar_at(nested_start + timedelta(minutes=9), 107.5, 107.6, 106.3, 106.8))  # fills the 106.6 limit
    return bars
...
def test_funnel_stats_show_anchor_but_no_nested_fvg_afterward():
    ...
    qualifying nested 1m FVG inside it should show up as a near-miss:
    breakout + anchor counted, zero nested_1m_fvgs, zero fills."""
    cfg = load_test_config()
    bars = [
        bar_at(PREV_DAY_BASE, 100.0, 101.0, 99.0, 100.0),
        bar_at(PREV_DAY_BASE + timedelta(minutes=15), 100.0, 105.0, 100.0, 104.0),
    ...
    for i in range(20):
        bars.append(flat_bar(flat_after + timedelta(minutes=i), 109.5, spread=0.3))  # flat, no 1m FVG ever forms
...
def test_export_chart_json_writes_candles_and_levels(tmp_path):
    ...
    assert trade["fvg_gap_low"] == pytest.approx(106.2)
    assert trade["fvg_gap_high"] == pytest.approx(107.0)
